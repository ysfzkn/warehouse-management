...
export default function StoreLayout() {
  ...
  const {
    warningOpen: sessWarn,
    secondsLeft: sessSec,
    dismiss: sessDismiss,
    logoutNow: sessLogout,
  } = useSessionGuard({
  ...
  useEffect(() => {
    console.log(
      '[StoreLayout] assistantFlags changed:',
      assistantFlags,
      '→ widget will render:',
      assistantFlags.storeEnabled === true
    );
  }, [assistantFlags]);
    ...
    <ToastProvider>
      <WishlistProvider>
        ...
        >
           ...
           */}
          {assistantFlags.storeEnabled === true && (
            <AssistantWidget config={storeAssistantConfig} siteName={siteSettings.get('site_name', '')} />
          )}
          <SessionWarningModal
            open={sessWarn}
            secondsLeft={sessSec}
