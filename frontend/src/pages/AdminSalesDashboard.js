...
export default function AdminSalesDashboard() {
  ...
  useEffect(() => {
    const fetchLive = () =>
      axios
        .get('/api/admin/sales-dashboard/live')
    ...
    datasets: [
      {
        ...
        backgroundColor: pmColors.slice(0, paymentBreakdown.length),
        borderWidth: 0,
      },
    ],
  };
          ...
          {liveStats && (
            ...
            >
              <span
                ...
                style={{ width: 8, height: 8, background: '#10b981', animation: 'pulse 2s infinite' }}
              />
              <span style={{ fontSize: 11, fontWeight: 600, color: '#059669' }}>
                Bugün: {fmt(liveStats.todayRevenue)} · {liveStats.todayOrders} sipariş
              </span>
            </div>
          )}
            ...
            {[
              ...
              <button
                ...
                onClick={() => {
                  setPeriod(p);
                  setChartDays(
                    p === 'today' ? 1 : p === 'week' ? 7 : p === 'month' ? 30 : p === 'quarter' ? 90 : 365
                  );
                }}
              >
                {label}
      ...
      <div className="row g-3 mb-4">
        {[
          ...
          },
          {
            ...
            value: fmtNum(s.periodOrders || s.totalOrders),
      ...
      <div className="card border-0 shadow-sm mb-4" style={{ borderRadius: 14 }}>
        <div className="card-body py-3">
          <div className="d-flex flex-wrap gap-3 justify-content-center">
            {[
              ['PENDING_PAYMENT', s.pendingPayment],
              ['PAID', s.paid],
              ['PREPARING', s.preparing],
              ['SHIPPED', s.shipped],
              ['DELIVERED', s.delivered],
              ['CANCELLED', s.cancelled],
              ['RETURN_REQUESTED', s.returnRequested],
            ].map(([status, count]) => (
              <div
                key={status}
              ...
              >
                <div className="fw-bold" style={{ color: STATUS_COLORS[status], fontSize: 20 }}>
                  {count || 0}
                </div>
          ...
          <div className="card border-0 shadow-sm" style={{ borderRadius: 14 }}>
            ...
            <div className="card-body pt-0" style={{ height: 300 }}>
              {dailyRevenue.length > 0 ? (
                ...
                <div className="text-center text-muted py-5">Veri yok</div>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="row g-4 mb-4">
        <div className="col-lg-6">
          <div className="card border-0 shadow-sm" style={{ borderRadius: 14 }}>
            <div className="card-header bg-transparent border-0">
              <h6 className="mb-0 fw-bold">
                <i className="fas fa-fire me-2 text-danger" />
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table className="w-100" style={{ borderCollapse: 'separate', borderSpacing: 2 }}>
                    ...
                    <tbody>
                      {dayNames.map((day, d) => (
                        <tr key={d}>
                          <td style={{ fontSize: 9, color: '#64748b', fontWeight: 600, paddingRight: 6 }}>
                            {day}
                          </td>
                          {Array.from({ length: 24 }, (_, h) => {
        ...
        <div className="col-lg-6">
          <div className="card border-0 shadow-sm" style={{ borderRadius: 14 }}>
            <div className="card-header bg-transparent border-0">
              <h6 className="mb-0 fw-bold">
                <i className="fas fa-trophy me-2 text-warning" />
        ...
        <div className="col-lg-6">
          <div className="card border-0 shadow-sm" style={{ borderRadius: 14 }}>
            <div className="card-header bg-transparent border-0 d-flex justify-content-between align-items-center">
              ...
              <button className="btn btn-sm btn-outline-primary" onClick={() => navigate('/admin/orders')}>
                Tümü
              </button>
            </div>
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted" style={{ fontSize: 10 }}>
                          {o.customerName} · {PM_LABELS[o.paymentMethod] || o.paymentMethod}
                        </div>
                      </div>
        ...
        <div className="col-lg-6">
          {paymentBreakdown.length > 0 && (
            <div className="card border-0 shadow-sm" style={{ borderRadius: 14 }}>
              <div className="card-header bg-transparent border-0">
                <h6 className="mb-0 fw-bold">
                  <i className="fas fa-chart-pie me-2" style={{ color: '#7c3aed' }} />
                  Ödeme Detayları
                </h6>
              </div>
              <div className="card-body p-0">
