...
const STATUS_CONFIG = {
  ...
  APPROVED:  { label: 'Onaylandı',  color: 'success',   icon: 'fas fa-check-circle',          desc: 'GİB tarafından onaylandı' },
  REJECTED:  { label: 'Reddedildi', color: 'danger',    icon: 'fas fa-times-circle',          desc: 'GİB tarafından reddedildi' },
  CANCELLED: { label: 'İptal',      color: 'dark',      icon: 'fas fa-ban',                   desc: 'Fatura iptal edildi' },
  ERROR:     { label: 'Hata',       color: 'danger',    icon: 'fas fa-exclamation-triangle',  desc: 'Gönderim sırasında hata' },
};
...
const formatDateShort = (d) => d ? new Date(d).toLocaleDateString('tr-TR', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '\u2014';
...
export default function AdminInvoices() {
  ...
  const buildCsv = (rows) => {
    const header = ['Fatura No', 'Sipariş No', 'Alıcı', 'Vergi No', 'Tip', 'Durum', 'Ara Toplam', 'KDV', 'Toplam', 'Oluşturulma'];
    const all = [header, ...rows.map(i => [
      i.invoiceNumber || '',
      i.orderNumber || '',
      i.recipientName || '',
      i.recipientTaxId || '',
  ...
  const handleExportCsv = () => {
    ...
    downloadCsv(buildCsv(invoices), `faturalar-sayfa-${page + 1}-${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const clearAllFilters = () => {
    setStatusFilter(''); setTypeFilter(''); setSearch(''); setDateFrom(''); setDateTo('');
  };
  ...
  const SortIcon = ({ field }) => {
    if (sortBy !== field) return <i className="fas fa-sort text-muted ms-1 opacity-50" style={{fontSize:10}}></i>;
    return <i className={`fas fa-sort-${sortDir === 'asc' ? 'up' : 'down'} text-primary ms-1`} style={{fontSize:10}}></i>;
  };
    ...
    <div>
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
        <div>
          ...
          <p className="text-muted small mb-0">
            ...
            {hasActiveFilters && <span className="ms-2 badge bg-warning bg-opacity-25 text-warning-emphasis border border-warning-subtle">{activeFilterCount} filtre aktif</span>}
          </p>
        </div>
        <div className="d-flex gap-2">
          <div className="dropdown">
            <button className="btn btn-sm btn-outline-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" disabled={totalElements === 0}
              title="Filtreli fatura listesini Excel'de açılabilen CSV olarak indir">
              <i className="fas fa-file-csv me-1"></i>CSV İndir
            </button>
            <ul className="dropdown-menu dropdown-menu-end">
              <li><button className="dropdown-item" onClick={handleExportCsv} disabled={invoices.length === 0}>
                <i className="fas fa-file-alt me-2 text-muted"></i>
                <span>Bu sayfayı indir <span className="text-muted small">({invoices.length} fatura)</span></span>
              </button></li>
              <li><button className="dropdown-item" onClick={handleExportAllCsv} disabled={totalElements === 0}>
                <i className="fas fa-download me-2 text-primary"></i>
                <span>Tüm filtreli sonuçları indir <span className="text-muted small">({totalElements} fatura)</span></span>
              </button></li>
              <li><hr className="dropdown-divider" /></li>
              <li><span className="dropdown-item-text small text-muted" style={{ maxWidth: 280 }}>
      ...
      <div className="row g-3 mb-4">
        {[
          ...
          { key: 'APPROVED',  label: 'Onaylı',     count: stats.approved, color: 'success',   icon: 'fas fa-check-circle' },
          { key: 'PENDING',   label: 'Beklemede',  count: stats.pending,  color: 'info',      icon: 'fas fa-hourglass-half' },
          { key: 'DRAFT',     label: 'Taslak',     count: stats.draft,    color: 'warning',   icon: 'fas fa-edit' },
          { key: 'ERROR',     label: 'Hatalı',     count: stats.error,    color: 'danger',    icon: 'fas fa-exclamation-triangle' },
        ].map(card => {
        ...
        <div className="card-body">
          <div className="row g-2 align-items-end">
            <div className="col-12 col-md-4">
              <label className="form-label small text-muted mb-1 d-flex align-items-center justify-content-between" style={{ minHeight: 20 }}>
                <span><i className="fas fa-search me-1"></i>Ara</span>
                <kbd className="text-muted bg-light border" style={{ fontSize: 10, padding: '1px 6px', borderRadius: 3 }}>/</kbd>
              </label>
              <div className="input-group input-group-sm">
                ...
                />
                {search && (
                  <button className="btn btn-outline-secondary" onClick={() => setSearch('')} title="Aramayı temizle" type="button">
            ...
            <div className="col-6 col-md-2">
              <label className="form-label small text-muted mb-1 d-block" style={{ minHeight: 20 }}>Tip</label>
            ...
            <div className="col-6 col-md-2">
              <label className="form-label small text-muted mb-1 d-block" style={{ minHeight: 20 }}>Başlangıç</label>
              <input type="date" className="form-control form-control-sm" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} />
            </div>
            <div className="col-6 col-md-2">
              <label className="form-label small text-muted mb-1 d-block" style={{ minHeight: 20 }}>Bitiş</label>
              <input type="date" className="form-control form-control-sm" value={dateTo} onChange={(e) => setDateTo(e.target.value)} />
            </div>
            <div className="col-6 col-md-2">
              <label className="form-label small text-muted mb-1 d-block" style={{ minHeight: 20, visibility: hasActiveFilters ? 'visible' : 'hidden' }}>&nbsp;</label>
              {hasActiveFilters ? (
            ...
            <div className="d-flex flex-wrap gap-2 mt-3 pt-3 border-top">
              {statusFilter && (
                <span className="badge bg-light text-dark border d-flex align-items-center gap-1 py-2 px-2">
                  ...
                  <button className="btn-close btn-close-sm ms-1" style={{fontSize:10}} onClick={() => setStatusFilter('')}></button>
                </span>
              )}
              {typeFilter && (
                <span className="badge bg-light text-dark border d-flex align-items-center gap-1 py-2 px-2">
                  ...
                  <button className="btn-close btn-close-sm ms-1" style={{fontSize:10}} onClick={() => setTypeFilter('')}></button>
                </span>
              )}
              {search && (
                <span className="badge bg-light text-dark border d-flex align-items-center gap-1 py-2 px-2">
                  ...
                  <button className="btn-close btn-close-sm ms-1" style={{fontSize:10}} onClick={() => setSearch('')}></button>
                </span>
              )}
              {dateFrom && (
                <span className="badge bg-light text-dark border d-flex align-items-center gap-1 py-2 px-2">
                  ...
                  <button className="btn-close btn-close-sm ms-1" style={{fontSize:10}} onClick={() => setDateFrom('')}></button>
                </span>
              )}
              {dateTo && (
                <span className="badge bg-light text-dark border d-flex align-items-center gap-1 py-2 px-2">
                  ...
                  <button className="btn-close btn-close-sm ms-1" style={{fontSize:10}} onClick={() => setDateTo('')}></button>
                </span>
              )}
            </div>
          )}
      ...
      <div className="card border-0 shadow-sm">
        <div className="card-body p-0">
          {loading ? (
            <div className="p-3">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="d-flex align-items-center gap-3 py-2 border-bottom" style={{ opacity: 0.5 }}>
              ...
              <table className="table table-hover mb-0 align-middle">
                <thead className="table-light" style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                  <tr>
                    ...
                    <th style={{ cursor: 'pointer', userSelect: 'none' }} onClick={() => toggleSort('invoiceNumber')}>
                      Fatura No <SortIcon field="invoiceNumber" />
                    </th>
                    <th style={{ cursor: 'pointer', userSelect: 'none' }} onClick={() => toggleSort('orderNumber')}>
                      Sipariş <SortIcon field="orderNumber" />
                    </th>
                    <th>Alıcı</th>
                    <th>Tip</th>
                    <th style={{ cursor: 'pointer', userSelect: 'none' }} onClick={() => toggleSort('totalAmount')}>
                ...
                <tbody>
                  {invoices.map((inv) => {
                    ...
                    return (
                      <tr key={inv.id} style={{ cursor: 'pointer', backgroundColor: isSelected ? 'rgba(13,110,253,0.05)' : undefined }} onClick={() => setSelectedInvoice(inv)}>
                        <td onClick={(e) => e.stopPropagation()}>
                          <input type="checkbox" className="form-check-input" checked={isSelected} onChange={() => toggleSelect(inv.id)} />
                        </td>
                        <td>
                          <div className="d-flex align-items-center gap-2">
                            <span className="fw-semibold">{inv.invoiceNumber || <span className="text-muted fst-italic">Atanmadı</span>}</span>
                            {inv.invoiceNumber && (
                              <button
                                type="button"
                                className="btn btn-sm btn-link p-0 text-muted"
                                onClick={(e) => { e.stopPropagation(); handleCopy(inv.invoiceNumber, `num-${inv.id}`); }}
                                title="Kopyala"
                              >
                                <i className={`fas ${copiedId === `num-${inv.id}` ? 'fa-check text-success' : 'fa-copy'}`} style={{ fontSize: 11 }}></i>
                              </button>
                            )}
                          </div>
                        </td>
                        <td>
                          <span className="badge bg-light text-dark border font-monospace">{inv.orderNumber}</span>
                        </td>
                        <td className="text-truncate" style={{ maxWidth: 180 }} title={inv.recipientName}>
                          {inv.recipientName || <span className="text-muted">—</span>}
                          {inv.recipientTaxId && (
                            <div className="small text-muted">{inv.recipientTaxId}</div>
                          )}
                        </td>
                        <td>
                          <span className={`badge bg-${typeCfg.color} bg-opacity-10 text-${typeCfg.color} border border-${typeCfg.color}-subtle`}>
                            <i className={`${typeCfg.icon} me-1`}></i>{typeCfg.label}
                          </span>
                        </td>
                        <td className="fw-semibold">{formatPrice(inv.totalAmount)}</td>
                        <td>
                          <span className={`badge bg-${statusCfg.color}`} title={statusCfg.desc}>
                            <i className={`${statusCfg.icon} me-1`}></i>{statusCfg.label}
                          </span>
                        </td>
                        <td>
                        ...
                        <td className="text-end" onClick={(e) => e.stopPropagation()}>
                          <div className="btn-group btn-group-sm">
                            {inv.hasPdf && (
                              <button className="btn btn-outline-primary" title="PDF İndir" onClick={() => handleDownloadPdf(inv.id, inv.invoiceNumber)}>
                                <i className="fas fa-download"></i>
                              </button>
                            )}
                            {(inv.status === 'ERROR' || inv.status === 'DRAFT' || inv.status === 'REJECTED') && (
                              <button className="btn btn-outline-warning" title="Yeniden Oluştur" disabled={actionLoading}
                                onClick={() => setConfirmAction({ type: 'regenerate', id: inv.id, title: 'Yeniden Oluştur', message: `${inv.invoiceNumber || 'Bu'} faturayı yeniden oluşturmak istiyor musunuz?`, btnClass: 'btn-warning', btnLabel: 'Yeniden Oluştur' })}>
                                <i className="fas fa-redo"></i>
                              </button>
                            )}
                            {inv.status !== 'CANCELLED' && (
                              <button className="btn btn-outline-danger" title="İptal Et" disabled={actionLoading}
                                onClick={() => setConfirmAction({ type: 'cancel', id: inv.id, title: 'Faturayı İptal Et', message: `${inv.invoiceNumber || 'Bu'} faturayı iptal etmek istediğinize emin misiniz? Bu işlem geri alınamaz.`, btnClass: 'btn-danger', btnLabel: 'İptal Et' })}>
                ...
                {invoices.length > 0 && (
                  <tfoot className="table-light">
                    <tr>
                      <td colSpan="5" className="text-end fw-semibold">Sayfa Toplamı:</td>
                      <td className="fw-bold text-primary" colSpan="4">{formatPrice(stats.totalSum)}</td>
                    </tr>
                  </tfoot>
                )}
              </table>
        ...
        {totalPages > 1 && (
          <div className="card-footer bg-white border-top d-flex flex-wrap justify-content-between align-items-center gap-2">
            <span className="text-muted small">
              Sayfa <strong>{page + 1}</strong> / {totalPages} <span className="text-muted">({totalElements} fatura)</span>
            </span>
            <div className="btn-group btn-group-sm">
              <button className="btn btn-outline-primary" disabled={page === 0} onClick={() => fetchInvoices(0)} title="İlk sayfa">
                <i className="fas fa-angle-double-left"></i>
              </button>
              <button className="btn btn-outline-primary" disabled={page === 0} onClick={() => fetchInvoices(page - 1)}>
                <i className="fas fa-chevron-left"></i> Önceki
              </button>
              <button className="btn btn-outline-primary" disabled={page >= totalPages - 1} onClick={() => fetchInvoices(page + 1)}>
                Sonraki <i className="fas fa-chevron-right"></i>
              </button>
              <button className="btn btn-outline-primary" disabled={page >= totalPages - 1} onClick={() => fetchInvoices(totalPages - 1)} title="Son sayfa">
        ...
        >
          <div className="modal-dialog modal-lg modal-dialog-scrollable" onClick={(e) => e.stopPropagation()}>
            <div className="modal-content">
              <div className={`modal-header bg-${(STATUS_CONFIG[selectedInvoice.status] || STATUS_CONFIG.DRAFT).color} bg-opacity-10 border-bottom`}>
                <div>
                  <h5 className="modal-title d-flex align-items-center gap-2">
                    <i className="fas fa-file-invoice"></i>
                    Fatura Detayı
                    {selectedInvoice.invoiceNumber && (
                      <span className="badge bg-light text-dark border font-monospace">{selectedInvoice.invoiceNumber}</span>
                    )}
                  </h5>
              ...
              <div className="modal-body">
                ...
                <div className="row g-3">
                  <div className="col-md-6">
                    <h6 className="text-muted text-uppercase small mb-3"><i className="fas fa-info-circle me-1"></i>Fatura Bilgileri</h6>
                    ...
                    <div className="mb-3">
                      ...
                      <div>
                        {(() => {
                          ...
                          return <span className={`badge bg-${tc.color}`}><i className={`${tc.icon} me-1`}></i>{tc.label}</span>;
                        })()}
                      </div>
                    </div>
                    <div className="mb-3">
                    ...
                    {selectedInvoice.providerInvoiceId && (
                      <div className="mb-3">
                        ...
                        <div className="font-monospace small d-flex align-items-center gap-2">
                          {selectedInvoice.providerInvoiceId}
                          <button className="btn btn-sm btn-link p-0" onClick={() => handleCopy(selectedInvoice.providerInvoiceId, 'provid')}>
                            <i className={`fas ${copiedId === 'provid' ? 'fa-check text-success' : 'fa-copy text-muted'}`} style={{fontSize:11}}></i>
                          </button>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="col-md-6">
                    <h6 className="text-muted text-uppercase small mb-3"><i className="fas fa-user me-1"></i>Alıcı Bilgileri</h6>
                    ...
                    {selectedInvoice.recipientTaxId && (
                      <div className="mb-3">
                        ...
                        <div className="d-flex align-items-center gap-2">
                          ...
                          <button className="btn btn-sm btn-link p-0" onClick={() => handleCopy(selectedInvoice.recipientTaxId, 'taxid')}>
                            <i className={`fas ${copiedId === 'taxid' ? 'fa-check text-success' : 'fa-copy text-muted'}`} style={{fontSize:11}}></i>
                          </button>
                        </div>
                      </div>
                    )}
                ...
                <h6 className="text-muted text-uppercase small mb-3"><i className="fas fa-calculator me-1"></i>Tutarlar</h6>
                ...
                {/* Timeline */}
                <hr />
                <h6 className="text-muted text-uppercase small mb-3"><i className="fas fa-history me-1"></i>Zaman Çizelgesi</h6>
                <div className="timeline">
                  <div className="d-flex gap-3 mb-2">
                    <div className="rounded-circle bg-secondary bg-opacity-25 d-flex align-items-center justify-content-center" style={{width:32,height:32,minWidth:32}}>
                  ...
                  {selectedInvoice.issuedAt && (
                    <div className="d-flex gap-3 mb-2">
                      <div className="rounded-circle bg-success bg-opacity-25 d-flex align-items-center justify-content-center" style={{width:32,height:32,minWidth:32}}>
                  ...
                  {selectedInvoice.updatedAt && selectedInvoice.updatedAt !== selectedInvoice.createdAt && (
                    <div className="d-flex gap-3">
                      <div className="rounded-circle bg-info bg-opacity-25 d-flex align-items-center justify-content-center" style={{width:32,height:32,minWidth:32}}>
              ...
              <div className="modal-footer">
                {selectedInvoice.hasPdf && (
                  <button className="btn btn-primary" onClick={() => handleDownloadPdf(selectedInvoice.id, selectedInvoice.invoiceNumber)}>
                    <i className="fas fa-download me-1"></i>PDF İndir
                  </button>
                )}
                {(selectedInvoice.status === 'ERROR' || selectedInvoice.status === 'DRAFT' || selectedInvoice.status === 'REJECTED') && (
                  <button className="btn btn-warning" disabled={actionLoading}
                    onClick={() => setConfirmAction({ type: 'regenerate', id: selectedInvoice.id, title: 'Yeniden Oluştur', message: `${selectedInvoice.invoiceNumber || 'Bu'} faturayı yeniden oluşturmak istiyor musunuz?`, btnClass: 'btn-warning', btnLabel: 'Yeniden Oluştur' })}>
                    <i className="fas fa-redo me-1"></i>Yeniden Oluştur
                  </button>
                )}
                {selectedInvoice.status !== 'CANCELLED' && (
                  <button className="btn btn-outline-danger" disabled={actionLoading}
                    onClick={() => setConfirmAction({ type: 'cancel', id: selectedInvoice.id, title: 'Faturayı İptal Et', message: `${selectedInvoice.invoiceNumber || 'Bu'} faturayı iptal etmek istediğinize emin misiniz? Bu işlem geri alınamaz.`, btnClass: 'btn-danger', btnLabel: 'İptal Et' })}>
      ...
      {/* Confirm Dialog */}
      {confirmAction && (
        <div className="modal show d-block" tabIndex="-1" style={{ backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 1060 }} onClick={() => setConfirmAction(null)}>
          <div className="modal-dialog modal-dialog-centered modal-sm" onClick={(e) => e.stopPropagation()}>
            <div className="modal-content">
              <div className="modal-body text-center py-4">
                <div className={`rounded-circle bg-${confirmAction.btnClass.replace('btn-','')} bg-opacity-10 d-inline-flex align-items-center justify-content-center mb-3`} style={{width:60,height:60}}>
                  <i className={`fas fa-exclamation text-${confirmAction.btnClass.replace('btn-','')} fa-2x`}></i>
                </div>
              ...
              <div className="modal-footer border-0 justify-content-center pt-0">
                <button className="btn btn-outline-secondary" onClick={() => setConfirmAction(null)} disabled={actionLoading}>Vazgeç</button>
                <button
                  className={`btn ${confirmAction.btnClass}`}
                  disabled={actionLoading}
                  onClick={() => confirmAction.type === 'regenerate' ? handleRegenerate(confirmAction.id) : handleCancel(confirmAction.id)}
                >
      ...
      {toast.show && (
        <div className="position-fixed bottom-0 end-0 p-3" style={{ zIndex: 9999 }}>
          <div className={`toast show align-items-center text-white bg-${toast.type} border-0`} style={{ minWidth: 280 }}>
            <div className="d-flex">
              <div className="toast-body">
                <i className={`fas ${toast.type === 'success' ? 'fa-check-circle' : toast.type === 'danger' ? 'fa-times-circle' : toast.type === 'warning' ? 'fa-exclamation-triangle' : 'fa-info-circle'} me-2`}></i>
                {toast.message}
              </div>
