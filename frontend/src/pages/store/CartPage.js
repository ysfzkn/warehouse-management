...
export default function CartPage() {
  ...
  return (
    <div className="container my-4 store-cart-has-sticky">
      ...
      <div className="row g-4">
                ...
                <div className="card-body d-flex gap-3 align-items-center p-3">
                  ...
                  <Link to={`/urun/${item.productSlug}`} className="flex-shrink-0 position-relative">
                    ...
                    >
                      {item.imageUrl ? (
                        ...
                        <FiShoppingCart size={24} className="text-muted" />
                      )}
                    </div>
                    {hasDiscount && (
                      <span
                        className="badge bg-danger position-absolute"
                        style={{ top: -4, left: -4, fontSize: 9 }}
                      >
                        %{discountPct}
                      </span>
                    )}
                  ...
                  <div className="flex-grow-1 min-w-0">
                    ...
                    <div className="d-flex align-items-center gap-2 mb-2">
                      {hasDiscount ? (
                        <>
                          ...
                          <del className="text-muted" style={{ fontSize: 12 }}>
                    ...
                    {item.availableStock != null && item.availableStock < item.quantity && (
                      <small className="text-danger">
                        ...
                        Stokta sadece {item.availableStock} adet var
                      </small>
                    )}
                  </div>
                  ...
                  >
                    <div
                      ...
                      style={{ border: '1.5px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}
                    >
                      <button
                        className="btn btn-sm d-flex align-items-center justify-content-center"
                        style={{ width: 40, height: 44, border: 'none', background: '#f8fafc' }}
                        onClick={() => {
                          hapticTap();
                          cart.updateItem(item.id, Math.max(1, item.quantity - 1));
                        }}
        ...
        <div className="col-lg-4">
          <div className="store-cart-summary">
            ...
            <div className="store-cart-summary-row">
              <span>Kargo</span>
              <span className={cart.cart.shippingCost > 0 ? '' : 'text-success fw-medium'}>
                {cart.cart.shippingCost > 0 ? formatPrice(cart.cart.shippingCost) : 'Ücretsiz'}
              </span>
            </div>
            {cart.cart.discountAmount > 0 && (
              <div className="store-cart-summary-row text-success">
                ...
                <span>-{formatPrice(cart.cart.discountAmount)}</span>
              </div>
            )}
            {/* Compute total discount savings */}
            {(() => {
              const totalSaved = (cart.cart?.items || []).reduce((sum, i) => {
                if (i.salePrice && i.salePrice > 0 && i.salePrice < i.unitPrice)
                  return sum + (i.unitPrice - i.salePrice) * i.quantity;
                return sum;
              }, 0);
              return totalSaved > 0 ? (
                <div className="store-cart-summary-row">
                  ...
                  <span className="text-danger fw-bold">-{formatPrice(totalSaved)}</span>
                </div>
              ) : null;
            })()}
      ...
      {/* Mobil sticky alt bar — toplam + CTA hep görünür */}
      <div className="store-cart-sticky-bar d-md-none">
        <div className="store-cart-sticky-info">
          <small>Toplam</small>
          <strong>{formatPrice(cart.cart.total)}</strong>
        </div>
