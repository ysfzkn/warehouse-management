...
import { isGoogleAuthEnabled, startGoogleAuth } from '../../utils/googleAuth';
...
export default function StoreRegisterPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!allPwdValid) {
      toast.warning('Şifre gereksinimleri karşılanmıyor.');
      return;
    }
    if (!form.termsConsent) {
      toast.warning('Üyelik sözleşmesini kabul etmeniz gerekmektedir.');
      return;
    }
    if (!form.kvkkConsent) {
      toast.warning('Kişisel verilerin işlenmesine ilişkin aydınlatma metnini onaylamanız gerekmektedir.');
      return;
    }
    setLoading(true);
    setError('');
  ...
  const handleGoogleRegister = () => {
    if (!startGoogleAuth()) {
      toast.warning('Google ile kayıt şu an kullanılamıyor.');
    }
  };
        ...
        <div className="card border-0 shadow-lg">
          <div className="card-body text-center p-5">
            ...
            <p className="text-muted mb-4">
              <strong>{form.email}</strong> adresine bir aktivasyon e-postası gönderdik. Hesabınızı
              etkinleştirmek için e-postanızdaki bağlantıya tıklayın.
            </p>
            <div className="alert alert-info small text-start">
              <i className="fas fa-info-circle me-2" />
              E-posta gelmedi mi? Lütfen spam veya gereksiz e-posta klasörünüzü kontrol edin. E-posta birkaç
              dakika içerisinde ulaşacaktır.
            </div>
    ...
    <div className="container py-5" style={{ maxWidth: 520 }}>
      <div className="card border-0 shadow-lg overflow-hidden">
        ...
        <div className="card-body p-4">
          {/* Register with Google — hidden on a build with no client id. */}
          {isGoogleAuthEnabled() && (
            <>
              <button
                type="button"
              ...
              <div className="d-flex align-items-center gap-3 mb-3">
                ...
                <span className="text-muted small">veya e-posta ile</span>
                <hr className="flex-grow-1" />
              </div>
            </>
          )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="row g-3 mb-3">
              <div className="col-6">
                ...
                <input
                  ...
                  onBlur={(e) => setForm((prev) => ({ ...prev, firstName: toTitleCaseTr(e.target.value) }))}
                  required
                  minLength={2}
                  placeholder="Adınız"
                />
              ...
              <div className="col-6">
                ...
                <input
                  ...
                  value={form.lastName}
                  onChange={handleChange}
                  onBlur={(e) => setForm((prev) => ({ ...prev, lastName: toTitleCaseTr(e.target.value) }))}
                  required
                  minLength={2}
            ...
            <div className="mb-3">
              ...
              <div className="input-group">
                <span className="input-group-text fw-medium" style={{ minWidth: 56 }}>
                  +90
                </span>
                <input
                  type="tel"
              ...
              {form.password.length > 0 && (
                <div className="mt-2 p-2 bg-light rounded">
                  {PWD_RULES.map((rule) => (
                    <div
                      key={rule.key}
                      className={`small d-flex align-items-center gap-2 ${rule.test(form.password) ? 'text-success' : 'text-muted'}`}
                    >
                      <i
                        className={`fas ${rule.test(form.password) ? 'fa-check-circle' : 'fa-circle'}`}
                        style={{ fontSize: 10 }}
                      />
                      {rule.label}
                    </div>
                  ...
                  <div className="mt-2">
                    <div className="progress" style={{ height: 4 }}>
                      <div
                        className={`progress-bar ${allPwdValid ? 'bg-success' : pwdScore >= 2 ? 'bg-warning' : 'bg-danger'}`}
                    ...
                    <div className="text-end mt-1">
                      <small
                        className={
                          allPwdValid ? 'text-success' : pwdScore >= 2 ? 'text-warning' : 'text-danger'
                        }
                      >
            ...
            <div className="mb-3 p-3 bg-light rounded">
              <div className="form-check mb-2">
                ...
                <label className="form-check-label small" htmlFor="terms">
                  ...
                  'ni okudum ve kabul ediyorum. <span className="text-danger">*</span>
                </label>
              </div>
              <div className="form-check mb-2">
                <input
                ...
                <label className="form-check-label small" htmlFor="kvkk">
                  ...
                  <a href="/sayfa/kvkk" target="_blank" rel="noopener noreferrer" className="text-primary">
                    Aydınlatma Metni
                  </a>
                  'ni okudum. <span className="text-danger">*</span>
                </label>
              </div>
            ...
            <button
              ...
              disabled={loading || !allPwdValid || !form.kvkkConsent || !form.termsConsent}
            >
              {loading ? (
                <>
          ...
          <div className="text-center mt-4 pt-3 border-top">
            ...
            <Link to="/giris" className="fw-semibold small">
              Giriş Yapın
            </Link>
          </div>
        </div>
