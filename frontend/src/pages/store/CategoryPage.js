...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((r) => setColors((r.data || []).filter((c) => c.name)))
      .catch(() => {});
  }, []);
    ...
    <>
      ...
      <FilterSection title="Kategoriler" defaultOpen={true}>
        <div className="store-filter-list">
          ...
          >
            <span className="store-filter-radio">
              {!categoryId && <span className="store-filter-radio-dot" />}
            </span>
          ...
          {categories.map((cat) => (
            <React.Fragment key={cat.id}>
              ...
              >
                <span className="store-filter-radio">
                  {categoryId === cat.id && <span className="store-filter-radio-dot" />}
                </span>
                <span>{cat.name}</span>
              </label>
              {cat.children?.map((sub) => (
                ...
                >
                  <span className="store-filter-radio small">
                    {categoryId === sub.id && <span className="store-filter-radio-dot" />}
                  </span>
                  <span>{sub.name}</span>
                </label>
              ))}
          ...
          <div className="store-color-grid">
            {colors.map((c) => (
              <button
                ...
                onClick={() => {
                  setSelectedColors((prev) => toggleSet(prev, c.id));
                  setPage(0);
                }}
              >
              ...
              {Array.from(selectedColors).map((id) => {
                ...
                return c ? (
                  <span key={id} className="store-selected-color-tag">
                    ...
                    <button
                      onClick={() => {
                        setSelectedColors((prev) => toggleSet(prev, id));
                        setPage(0);
                      }}
                    >
                      <FiX size={10} />
      ...
      <FilterSection title="Fiyat Aralığı" count={priceApplied.min || priceApplied.max ? 1 : 0}>
        <div className="store-price-filter">
          ...
          {(priceApplied.min || priceApplied.max) && (
            <button className="store-filter-clear mt-1" onClick={clearPrice}>
      ...
      {onClose && (
        <div className="store-filter-actions">
          <button
            className="store-filter-action-clear"
            onClick={() => {
              clearAllFilters();
              onClose();
            }}
        ...
        <div className="col-lg-9">
            ...
            <div className="store-active-filters">
              ...
              {Array.from(selectedBrands).map((id) => {
                ...
                return b ? (
                  <span key={`b-${id}`} className="store-chip store-chip-brand">
                    ...
                    <button
                      onClick={() => {
                        setSelectedBrands((prev) => toggleSet(prev, id));
                        setPage(0);
                      }}
                    >
                      <FiX size={12} />
              ...
              {(priceApplied.min || priceApplied.max) && (
                <span className="store-chip store-chip-price">
                  {priceApplied.min ? `${priceApplied.min}₺` : '0₺'} —{' '}
                  {priceApplied.max ? `${priceApplied.max}₺` : '∞'}
              ...
              {activeFilterCount > 1 && (
                <button className="store-chip-clear-all" onClick={clearAllFilters}>
                  Tümünü Temizle
                </button>
              )}
            </div>
          ...
          {loading ? (
            ...
            <div className="store-empty-state">
              ...
              {activeFilterCount > 0 && (
                <button className="btn btn-outline-primary btn-sm" onClick={clearAllFilters}>
                  Filtreleri Temizle
                </button>
              )}
            </div>
            ...
            <div
              className={`row g-3 ${viewMode === 'list' ? 'row-cols-1' : 'row-cols-2 row-cols-md-3 row-cols-lg-3'}`}
            >
              {products.map((p) => (
