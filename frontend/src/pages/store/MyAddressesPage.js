...
export default function MyAddressesPage() {
  ...
  const handleSave = async (formData) => {
    try {
      if (editing) await axios.put(`/api/store/addresses/${editing}`, formData, { headers: getAuthHeaders() });
      else await axios.post('/api/store/addresses', formData, { headers: getAuthHeaders() });
      setShowForm(false); setEditing(null); setEditData(null); fetchAddresses();
      toast.success(editing ? 'Adres güncellendi.' : 'Adres kaydedildi.');
    } catch (e) { toast.error(e.response?.data?.message || 'Adres kaydedilemedi.'); }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      fetchAddresses(); toast.success('Adres silindi.');
  ...
  const handleSetDefault = async (id) => {
    try {
      ...
      fetchAddresses(); toast.success('Varsayılan adres güncellendi.');
  ...
  const startCreate = () => {
    setEditing(null); setEditData(null); setShowForm(true);
  };

  return (
    <div className="container py-4" style={{ maxWidth: 800 }}>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="fw-bold mb-0"><FiMapPin className="me-2 text-primary" />Adreslerim</h2>
        {!showForm && (
          <button className="btn btn-primary btn-sm" onClick={startCreate}>
            <FiPlus size={14} className="me-1" />Yeni Adres
          </button>
        )}
      </div>
      ...
      {showForm && (
        <div className="card border-0 shadow-sm mb-4" style={{ borderRadius: 16 }}>
          <div className="card-header bg-transparent border-0 d-flex justify-content-between align-items-center py-3">
            ...
            <button className="btn-close" onClick={() => { setShowForm(false); setEditing(null); setEditData(null); }} />
          </div>
          <div className="card-body pt-0">
            <AddressForm
              initialData={editData}
              onSubmit={handleSave}
              onCancel={() => { setShowForm(false); setEditing(null); setEditData(null); }}
      ...
      : addresses.length === 0 && !showForm ? (
        <div className="card border-0 shadow-sm" style={{ borderRadius: 16 }}>
          <div className="card-body text-center py-5">
            ...
            <button className="btn btn-primary btn-sm" onClick={startCreate}>
              <FiPlus size={14} className="me-1" />İlk Adresinizi Ekleyin
            </button>
          </div>
        </div>
      ) : (
        <div className="d-flex flex-column gap-3">
          {addresses.map(a => (
            <div key={a.id} className={`card border-0 shadow-sm ${a.isDefault ? 'border-start border-3 border-primary' : ''}`} style={{ borderRadius: 14 }}>
              <div className="card-body">
                <div className="d-flex justify-content-between align-items-start">
                  <div className="d-flex align-items-center gap-2 mb-2">
                    <span className="fw-bold">{a.title || 'Adres'}</span>
                    {a.isDefault && (
                      <span className="badge bg-primary" style={{ fontSize: 10 }}>Varsayılan</span>
                    )}
                  </div>
                  <div className="d-flex gap-1">
                    <button className="btn btn-sm btn-outline-primary" onClick={() => startEdit(a)} title="Düzenle" style={{ width: 32, height: 32, padding: 0 }}>
                      <FiEdit2 size={13} />
                    </button>
                    {!a.isDefault && (
                      <button className="btn btn-sm btn-outline-success" onClick={() => handleSetDefault(a.id)} title="Varsayılan yap" style={{ width: 32, height: 32, padding: 0 }}>
                        <FiStar size={13} />
                      </button>
                    )}
                    <button className="btn btn-sm btn-outline-danger" onClick={() => handleDelete(a.id)} title="Sil" style={{ width: 32, height: 32, padding: 0 }}>
                      <FiTrash2 size={13} />
                    </button>
                  </div>
                </div>
                ...
                <div className="small">
                  ...
                  {a.phone && <div className="text-muted"><FiPhone size={11} className="me-1" />{a.phone}</div>}
                ...
                {a.companyName && (
                  <div className="mt-2 pt-2 border-top small text-muted">
                    <FiCreditCard size={12} className="me-1" />{a.companyName}
