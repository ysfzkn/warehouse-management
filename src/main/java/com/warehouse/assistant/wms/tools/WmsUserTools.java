...
/**
 * Tools safe for non-admin WMS users (STOCK_IN, STOCK_OUT).
 */
@Component
public class WmsUserTools {

    private static final Logger log = LoggerFactory.getLogger(WmsUserTools.class);

    private final StockRequestService stockRequestService;
    private final StockTransferService stockTransferService;

    public WmsUserTools(StockRequestService stockRequestService,
    ...
    public PagedResponse<WmsAdminTools.WmsTransferListItem> myTransfers(String status,
                                                                         String transferType,
                                                                         ...
                                                                         Integer page,
                                                                         Integer size) {
        log.debug("WMS tool myTransfers filters: status={}, transferType={}, sourceWarehouseId={}, destinationWarehouseId={}, startDateRaw={}, endDateRaw={}, productName={}, sku={}, driverName={}, notes={}, customerQuery={}, page={}, size={}",
        ...
        filter.setStartDate(WmsAdminTools.parseDateTimeLoose(startDate));
        filter.setEndDate(WmsAdminTools.parseDateTimeLoose(endDate));
        ...
        if (transfers.getTotalElements() == 0
            ...
            log.debug("WMS tool myTransfers fallback: relaxed transferType filter (was {}). totalElements={}", filter.getTransferType(), transfers.getTotalElements());
        }
        List<WmsAdminTools.WmsTransferListItem> content = transfers.getContent().stream()
                .map(WmsAdminTools::toListItem)
                .toList();
    ...
    private static TransferStatus parseTransferStatus(String raw) {
        return WmsAdminTools.parseTransferStatus(raw);
    }

    private static TransferType parseTransferType(String raw) {
        return WmsAdminTools.parseTransferType(raw);
    }
}
